    ...
    try {
      ...
      if (userId != null) {
        ...
        if (_authPhoneNumber == null) {
          ...
          print("Retrieved auth phone number: $_authPhoneNumber");
        }
        ...
        try {
          ...
          print("Updated user record in 'users' collection");
        } catch (userUpdateError) {
          print('Error updating user record: $userUpdateError');
          
          // If the document doesn't exist, create it
          if (userUpdateError is FirebaseException && userUpdateError.code == 'not-found') {
            try {
              ...
              print("Created new user record in 'users' collection");
            } catch (createError) {
              print('Error creating user record: $createError');
              // Show error to user
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text('Failed to create user profile: $createError')),
              );
            }
          } else {
            // Show error to user
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('Failed to update user profile: $userUpdateError')),
            );
          }
        }
        ...
        try {
          ...
          print("Updated patient record in 'patients' collection");
        } catch (patientUpdateError) {
          print('Error updating patient record: $patientUpdateError');
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Failed to save patient data: $patientUpdateError')),
          );
        }
        ...
        if (_image != null) {
          try {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('Failed to upload profile image: $imageError')),
            );
          }
        }
      } else {
