...
class _FinancesScreenState extends State<FinancesScreen> {
  ...
  Future<void> _loadTransactions(String userId) async {
    try {
      ...
      // If no transactions collection or no documents, try appointments
  ...
  Future<void> _loadTransactionsFromAppointments(String userId) async {
    try {
      ...
      for (var doc in appointmentsSnapshot.docs) {
        ...
        String title = "Appointment with $patientName";
        double amount = data['fee'] is num ? (data['fee'] as num).toDouble() : 0.0;
        DateTime date = data['date'] is Timestamp 
            ? (data['date'] as Timestamp).toDate() 
            : DateTime.now();
        
        // Determine transaction type based on appointment status
        TransactionType type = data['status'] == 'completed' 
            ? TransactionType.income 
            : TransactionType.expense;
